...
const getAllReferral = catchAsync(async (req: Request, res: Response) => {
  const filters = pick(req.query, ['searchTerm', ...referralFilterAbleFields]);
...
const updateReferral: RequestHandler = catchAsync(
  async (req: Request, res: Response) => {
    ...
    const result = await ReferralService.updateReferral(id, updateAbleData);
...
const sendReferralEmail: RequestHandler = catchAsync(
  async (req: Request, res: Response) => {
    const data = req.body;
    const user = req.user as JwtPayload;

    const result = await ReferralService.sendReferralEmail(
      user.userId,
      data.sendTo
    );

    sendResponse<{ isSend: boolean }>(res, {
      statusCode: httpStatus.OK,
      success: true,
